...
- **Every Farm RPG item is now in the planner.** `data/extra-items.js`
  (generated by `tools/import-buddy.mjs`) adds the 418 items data.js was
  missing, their recipes, and Sinking Swamp (both sides) and Gary's Crushroom,
  merging itself into `FRPG_DATA` right after `data/data.js`. Rerun the tool
  when Farm RPG adds items; never hand-edit the generated file.
- Rates: drops per AP and per Large Net come from the owner's sheet
