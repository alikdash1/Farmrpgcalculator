...
(() => {
  ...
  function parseMasteryPage(lines, text) {
    ...
    for (let i = start + 1; i < clean.length; i++) {
      ...
      const progress = clean[i].match(/^([\d,]+)\s*\/\s*([\d,]+|∞)\s*Progress$/i);
      if (!progress || i === 0) continue;
      const name = clean[i - 1];
      if (!NAME_RE.test(name) || isNoise(name) || seen.has(name.toLowerCase())) continue;
      const currentNumber = Number(progress[1].replace(/,/g, ""));
      const targetNumber = progress[2] === "∞" ? null : Number(progress[2].replace(/,/g, ""));
  ...
  try {
    ...
    console.error("Farm RPG capture failed:", err);
    try { banner("Farm RPG capture failed: " + (err && err.message ? err.message : err), false); } catch (e2) { /* no body */ }
  }
})();
