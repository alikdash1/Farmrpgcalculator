...
(buddy.farm), or `override`. Resolution order: **personal > override > logged > almanac**.
...
### Personal rate collection (extension pipeline)

Phase-2 component built on the `coderanger/farmrpg-ext` browser extension, which
passively records explore/fish outcomes while Ali plays normally (large drink samples:
hundreds of ciders / Arnold Palmers included).

- `tools/import_logs.ps1`: converts extension log exports into
  `data/personal_rates.json` (`{location: {item: 1-in-N}}`) using the same math as the
  extension's own `py/droprates.py` (Wilson-score-friendly counts, not raw averages).
- Personal rates sit at the top of the trust order and are regenerated by re-running the
  import after more play — never hand-edited.
- v1 ships without this; the calculator already reads `personal_rates.json` when present,
  so enabling the pipeline later requires no code changes.
